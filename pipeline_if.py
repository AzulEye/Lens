...
class IFPipeline(DiffusionPipeline, LoraLoaderMixin):
    ...
    def encode_prompt(
        ...
        end_layer: int = None,
        step_layer: int = 1,
        explain_other_model: bool = False,
    ):
        r"""
        ...
        if prompt_embeds is None:
            ...
            print("prompt_embeds is not None")

        initial_negative_prompt_embeds = negative_prompt_embeds

        prompt_embeds_per_layer = []


        for layer_index, prompt_embeds in enumerate(hidden_states[start_layer:end_layer:step_layer]):
            negative_prompt_embeds = initial_negative_prompt_embeds
            ...
            if not (start_layer + layer_index * step_layer == len(hidden_states) - 1):
                ...
                print(f"layer: {start_layer + layer_index * step_layer} - not using final_layer_norm")
            ...
            prompt_embeds_per_layer.append((prompt_embeds, negative_prompt_embeds))


        return prompt_embeds_per_layer #, negative_prompt_embeds_per_layer
    ...
    def __call__(
        ...
        prompt_embeds_per_layer = self.encode_prompt(
            prompt,
            do_classifier_free_guidance,
            num_images_per_prompt=num_images_per_prompt,
            device=device,
        ...
        res = []
        for hs_index, (prompt_embeds, negative_prompt_embeds) in enumerate(prompt_embeds_per_layer):
            timesteps = None
            ...
            if not return_dict:
                return (image, nsfw_detected, watermark_detected)
            res.append(IFPipelineOutput(images=image,
                                        nsfw_detected=nsfw_detected,
                                        watermark_detected=watermark_detected))
        return res, prompt_embeds_per_layer
